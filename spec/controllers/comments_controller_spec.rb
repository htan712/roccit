...
RSpec.describe CommentsController, type: :controller do
  ...
  let(:other_user) { User.create!(name: Faker::Internet.user_name, email: Faker::Internet.safe_email, password: 'helloworld', role: :member) }
  ...
  context 'guest' do
    describe 'POST create' do
      it 'redirects the user to the sign in view' do
        post :create, post_id: my_post.id, comment: { body: Faker::Hipster.paragraph }
  ...
  context "member user doing CRUD on a comment they don't own" do
    ...
    describe 'POST create' do
      it 'increases the number of comments by 1' do
        expect { post :create, post_id: my_post.id, comment: { body: Faker::Hipster.sentence } }.to change(Comment, :count).by(1)
      end

      it 'redirects to the post show view' do
        post :create, post_id: my_post.id, comment: { body: Faker::Hipster.sentence }
